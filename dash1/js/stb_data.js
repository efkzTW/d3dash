...
var svg2 = d3.select("#tw-line").append("svg")
			.attr("width", width + margin.left + margin.right)
			.attr("height", height + margin.top + margin.bottom)
		.append("g")
			.attr("transform", "translate(" + margin.left + "," + margin.top + ")");
...
d3.csv("../data/stb_data.csv", function(error, data) {
	...
	y.domain(d3.extent(data1, function(d) {return d.total;}));
	...
	var totalMax = d3.max(data1, function(d){return d.total});
	var totalMin = d3.min(data1, function(d){return d.total});
	var totalAvg = Math.floor(d3.sum(data1, function(d){return d.total})/data1.length) + 1;
...
d3.csv("../data/stb_data.csv", function(error, data) {
	...
	data2 = data.filter(function(obj) {
		if (parseDate(obj.date) >= dateMinFilter && parseDate(obj.date) <= dateMaxFilter) {
			return obj;
		};
	});
	...
	y.domain(d3.extent(data2, function(d) {return d.total;}));
	...
	var totalMax = d3.max(data2, function(d){return d.total});
	var totalMin = d3.min(data2, function(d){return d.total});
	var totalAvg = Math.floor(d3.sum(data2, function(d){return d.total})/data2.length) + 1;
